...
This will:
- Add the MCP server under the default (local) scope ΓÇö meaning it's only available inside the current folder.
...
1. Open Cursor and open Command Pallet ( `Shift + Γîÿ + P` on Mac or `Ctrl+ Shift + P` on Windows/Linux )
...
1. Open the Command Palette (Γîÿ+Shift+P on Mac or Ctrl+Shift+P on Windows/Linux)
